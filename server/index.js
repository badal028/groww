...
const isIndexUnderlying = (underlyingNormalized) =>
...
const filterOptionRowsForUnderlying = (instruments, underlyingNormalized) => {
  const nameCandidatesUpper = new Set(
    (isIndexUnderlying(underlyingNormalized)
      ? getIndexNameCandidates(underlyingNormalized)
      : getStockNameCandidates(instruments, underlyingNormalized)
  ...
  return instruments.filter((r) => {
    ...
    if (!r.tradingsymbol || !r.expiry || !r.name) return false;
...
/** Nearest F&O expiry in the dropdown (today IST onward), matching options-chain UI. */
const resolveNearestDropdownExpiryIso = (optionRows, underlyingNormalized) => {
  ...
  let list = expiriesAll;
  if (bankNiftyMonthlyOnly) {
    const byMonth = new Map();
    for (const it of expiriesAll) {
      const key = it.iso.slice(0, 7);
      const prev = byMonth.get(key);
      if (!prev || it.iso > prev.iso) byMonth.set(key, it);
    }
    list = Array.from(byMonth.values()).sort((a, b) => a.iso.localeCompare(b.iso));
  }

  const todayIst = todayISOInIST();
  const upcoming = list.filter((e) => e.iso >= todayIst);
  return (upcoming.length > 0 ? upcoming : list)[0]?.iso || null;
};

const INDEX_FO_EXPIRY_TAG_NAMES = [
  { displayName: "NIFTY 50", normalized: "NIFTY 50" },
  { displayName: "BANK NIFTY", normalized: "BANK NIFTY" },
  { displayName: "SENSEX", normalized: "SENSEX" },
];
...
app.get("/api/index-fo-expiry-tags", ensureAuth, async (_req, res) => {
  try {
    const instruments = await loadKiteInstruments();
    const todayIst = todayISOInIST();
    const tags = {};

    for (const { displayName, normalized } of INDEX_FO_EXPIRY_TAG_NAMES) {
      const optionRows = filterOptionRowsForUnderlying(instruments, normalized);
      if (optionRows.length === 0) {
        tags[displayName] = false;
        continue;
      }
      const nearestIso = resolveNearestDropdownExpiryIso(optionRows, normalized);
      tags[displayName] = Boolean(nearestIso && nearestIso === todayIst);
    }

    return res.json({ status: "ok", todayIst, tags });
  } catch (error) {
    return res.status(500).json({ status: "error", message: error?.message || "Index expiry tags failed" });
  }
});
