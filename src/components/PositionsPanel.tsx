...
import { formatDeliveryProductLine } from "@/lib/productLine";
...
function productLine(p: PaperPosition): string {
  return formatDeliveryProductLine(p.instrumentType, p.symbol);
}
  ...
  const pnlCard = (
    <div className="flex items-center justify-between gap-3 rounded-2xl border border-border bg-card px-4 py-5 dark:bg-[#141819]">
      <div className="min-w-0 flex-1">
        ...
        <p
          className={cn(
            "mt-2 text-[16px] font-bold tabular-nums leading-none tracking-tight lg:text-[1.75rem]",
          ...
          const rowInner = (
            <>
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <p
                  className={cn(
                    "shrink-0 text-[0.80rem] font-semibold tabular-nums leading-none lg:text-lg",
