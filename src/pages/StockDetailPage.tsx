    ...
    <div className="min-h-screen bg-background pb-20 lg:pb-0">
      ...
      <div className="lg:flex lg:gap-8 lg:px-8 lg:py-6">
        ...
        <div className="px-4 py-4 lg:px-0 lg:flex-1">
          <div className="mb-1 flex h-8 w-8 items-center justify-center rounded bg-muted text-xs font-bold text-muted-foreground lg:h-10 lg:w-10 lg:text-sm">
            {stock.symbol.slice(0, 3)}
          </div>
          <h1 className="mt-2 text-lg font-semibold text-foreground lg:text-2xl">{stock.name}</h1>
          <p className="text-2xl font-bold text-foreground lg:text-3xl">₹{stock.price.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
          ...
          {/* Chart */}
          <div className="my-6">
            <svg viewBox={`0 0 ${width} ${height}`} className="w-full" preserveAspectRatio="none" style={{ maxHeight: '250px' }}>
              <defs>
              ...
              <polygon points={`0,${height} ${points} ${width},${height}`} fill="url(#chartGrad)" />
              <polyline points={points} fill="none" stroke={isPositive ? 'hsl(164 100% 41%)' : 'hsl(0 72% 51%)'} strokeWidth="2" />
            </svg>
          ...
          <div className="hidden lg:flex gap-3 mb-6">
            <button className="flex-1 rounded-lg bg-loss py-3 text-sm font-semibold text-primary-foreground hover:opacity-90 transition-opacity">
              Sell
            </button>
            <button className="flex-1 rounded-lg bg-primary py-3 text-sm font-semibold text-primary-foreground hover:opacity-90 transition-opacity">
              Buy
            </button>
          </div>
        </div>

        {/* Right: Info section (desktop sidebar) */}
        <div className="px-4 lg:px-0 lg:w-[380px] lg:flex-shrink-0">
          {/* Create SIP Card */}
          <div className="mb-6 flex items-center justify-between rounded-xl border border-border bg-card p-4 hover:shadow-sm transition-shadow cursor-pointer">
      ...
      <div className="fixed bottom-0 left-0 right-0 flex gap-3 border-t border-border bg-card p-4 lg:hidden">
