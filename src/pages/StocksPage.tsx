...
import { useIndexFoExpiryTags } from '@/hooks/useIndexFoExpiryTags';
import IndexExpiryLabel from '@/components/IndexExpiryLabel';
...
const StocksPage: React.FC = () => {
  ...
  const indexExpiryTags = useIndexFoExpiryTags(marketIndices.map((i) => i.name));
        ...
        <div className="flex gap-3 overflow-x-auto px-4 py-3 lg:px-0 lg:flex-wrap scrollbar-hide">
          {liveIndices.map(index => (
            <button
              ...
              className="w-[230px] min-w-[190px] max-w-[190px] flex-shrink-0 rounded-lg border border-border bg-card px-4 py-2.5 text-left transition-colors hover:border-primary/30"
            >
              <div className="flex items-start justify-between gap-2">
                <p className="min-w-0 flex-1 truncate text-xs font-medium text-muted-foreground">{index.name}</p>
                <IndexExpiryLabel show={indexExpiryTags[index.name]} />
              </div>
              <div className="mt-0.5 flex flex-nowrap items-baseline gap-1.5 overflow-x-auto scrollbar-hide">
                <span className="shrink-0 text-[0.75rem] font-[100] tabular-nums tracking-[-0.5px] text-foreground">
                  {index.value.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                </span>
                <span
                  className={`shrink-0 whitespace-nowrap text-[0.75rem] font-[100] tabular-nums tracking-[-1px] ${index.change >= 0 ? 'text-profit' : 'text-loss'}`}
                >
                  {index.change >= 0 ? '+' : ''}
          ...
          <div className="flex items-center gap-8 overflow-x-auto py-4 text-sm scrollbar-hide">
            {liveIndices.slice(0, 5).map((index) => (
              <button
                ...
                type="button"
                onClick={() => navigate(`/stock/${encodeURIComponent(index.name)}`)}
                className="relative flex min-w-[360px] flex-shrink-0 flex-nowrap items-center gap-2 rounded px-2 py-1.5 tabular-nums transition-colors hover:bg-muted/50"
              >
                <span className="max-w-[8rem] shrink-0 truncate font-semibold text-foreground">
                  {index.name.toUpperCase()}
                </span>
                <IndexExpiryLabel show={indexExpiryTags[index.name]} />
                <span className="shrink-0 whitespace-nowrap text-[0.75rem] font-[100] tabular-nums tracking-[-0.5px] text-foreground">
                  {index.value.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                </span>
                <span
                  className={`shrink-0 whitespace-nowrap text-[0.75rem] font-[100] tabular-nums tracking-[-1px] ${index.change >= 0 ? 'text-profit' : 'text-loss'}`}
